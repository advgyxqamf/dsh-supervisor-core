...
//
// ## 三条断言
//   G10-a 受管 daemon 脚本路径必须真实存在（经 srcpath 解析）
...
const srcpath = require(path.join(ROOT, 'src', 'platform', 'srcpath.js'));

// ── G10-a 受管 daemon 脚本必须存在 ──
console.log('== G10-a 受管 daemon 脚本可解析 ==');
{
  for (const kind of ['router', 'lan']) {
    const p = srcpath.daemonScript(kind);
...
console.log('== G10-b 无脆弱的跨层路径推算 ==');
{
  ...
  for (const f of files) {
    ...
    if (rel === path.join('src', 'platform', 'srcpath.js')) continue; // 提供者本身豁免
...
// ── G10-d 调用点必须走 srcpath（防改回直接推算）──
console.log('== G10-d 受管 daemon 路径经 srcpath 解析 ==');
{
  const cv = path.join(ROOT, 'src', 'guard', 'supervisor', 'control-view.js');
  const rv = path.join(ROOT, 'src', 'guard', 'supervisor', 'registry-view.js');
  const cvSrc = fs.readFileSync(cv, 'utf8');
  const rvSrc = fs.readFileSync(rv, 'utf8');
  check('G10-d control-view 经 srcpath 解析 daemon 脚本',
    /srcpath'\)\.daemonScript\(/.test(cvSrc), 'daemonScript(...)');
  check('G10-d registry-view 经 srcpath 解析 router daemon',
    /daemonScript\('router'\)/.test(rvSrc), 'daemonScript(\'router\')');
  check('G10-d registry-view 经 srcpath 解析 lan daemon',
    /daemonScript\('lan'\)/.test(rvSrc), 'daemonScript(\'lan\')');
