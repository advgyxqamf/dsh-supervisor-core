 ...
 * presents every `/api` request to the trust fence as a loopback authority, so
 * the full Web UI (including the settings panel) works from a phone/LAN
 * browser over direct access. Everything else (HTTP and upgrade route
 * registries, index transform taps, the single fallback seat) keeps the
 * upstream contract so the composition resolves `ctx.webServer` unchanged.
 *
 * There is deliberately NO access-token gate: any request on the bound
 * interfaces is served directly. A gate would intercept throwaway instances
 * that developers and workers start on other ports for testing, returning 401
 * to every unauthenticated request and corrupting the real feedback loop of
 * the project.
 */
...
export class WebServer extends Service {
    ...
    /**
     * Present every `/api` request to the downstream trust fence as a loopback
     * authority instead of the LAN address it arrived on. The harness pins the
     * configuration plane (settings/credentials/agentPreset, host.*,
     * llm.discoverModels) to loopback "until a real authentication layer
     * exists" (client-connection PRIVILEGED_METHODS). This presentation is what
     * lets a LAN/phone browser use the full UI — including the settings panel —
     * over direct remote access. The cross-site defenses (Origin/Sec-Fetch-Site)
     * still run. Both the HTTP and the upgrade path apply it.
