...
import { Button, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "../../framework/ui";
      ...
      <Card>
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/70 px-5 py-3.5">
          <div className="flex items-center gap-2">
            ...
            {r?.autostart ? <Pill tone="boot">自动启动</Pill> : null}
          </div>
          <Button
      ...
      {accs.length ? (
        <div className="flex min-h-0 flex-col border-t border-border/60">
          <div className={cn("grid items-center gap-2 border-b border-border/60 bg-muted/60 px-5 py-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground", accountGridCols)}>
            <span>{p.kind === "proxy" ? "账号实例" : "API Key"}</span>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[440px]">
        ...
        <div className="grid gap-3">
          ...
          {/* 添加新 Key */}
          <div className="grid gap-1.5">
            <label className="text-xs font-medium text-muted-foreground">添加 {isProxy ? "账号 Key / Command Code" : "API Key"}（每行一个，支持逗号/分号分隔）</label>
            <textarea
              className="h-20 w-full resize-none rounded-md border border-input bg-transparent px-3 py-2 font-mono text-xs outline-none focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/20"
      ...
      <DialogContent className="max-w-[420px]">
        ...
        <div className="grid gap-3">
          ...
          {mode === "preset" ? (
            ...
            <div className="grid gap-1.5">
              <label className="text-sm font-medium text-foreground">反代应用</label>
          ...
          <div className="grid gap-1.5">
            <label className="text-sm font-medium text-foreground">供应商名称（可选）</label>
            <input className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm outline-none focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/20"
              placeholder="自定义名称，如：Open Code ZEN" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
