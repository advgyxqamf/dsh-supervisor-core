        ...
        <div className="grid grid-cols-1 md:grid-cols-[minmax(0,5fr)_minmax(0,6fr)]">
          ...
          <div className="flex items-center bg-[image:var(--panel-accent-gradient)] px-6 py-5">
            <div className="grid w-full grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-4">
              <Metric icon={<Activity className="size-4" />} label="DSH 端口" value={s?.dshPort ? String(s.dshPort) : "—"} mono />
              <Metric icon={<TerminalSquare className="size-4" />} label="DSH 进程" value={s?.dshPid ? String(s.dshPid) : "—"} mono />
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border/60 px-6 py-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {!installed && !nBusy ? (
              ...
              <>
                <Button disabled={!running} onClick={() => void openWeb()} size="sm" variant="outline">
                  <ExternalLink className="size-4" />进入 DSH Web
                </Button>
            ...
            {installed && !nBusy ? (
              <>
                ...
                <Button className="h-[30px]" disabled={busy === "uni"} onClick={() => void uninstallDsh()} size="sm" variant="destructive">
