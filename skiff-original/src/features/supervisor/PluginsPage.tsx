...
import { Button, Checkbox, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "../../framework/ui";
          ...
          <div className="grid gap-2">
            {[["native", "原生实例"], ["all", "所有实例（含原生）"]].concat(targets.map((t) => ["id:" + t.id, t.name + "（" + t.id.slice(0, 8) + "…）"] as [string, string])).map(([val, label]) => (
              <label key={val} className={cn("flex cursor-pointer items-center gap-2 rounded-md border px-3 py-2 text-sm", target === val ? "border-primary/40 bg-primary/[0.05] text-foreground" : "border-border text-muted-foreground hover:bg-muted")}>
                <input type="radio" className="accent-primary" name="pTarget" checked={target === val} onChange={() => setTarget(val)} />
                {label}
              </label>
            ))}
          </div>
    ...
    <Card className="overflow-hidden">
      ...
      {!data ? <div className="py-16 text-center text-sm text-muted-foreground">加载中…</div>
        ...
        : (
          <>
            {/* 表头 */}
            <div className={cn(gridCols, "border-b border-border/70 bg-muted px-5 py-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground")}>
              <span>来源</span>
              <span>插件</span>
              <span>版本</span>
              ...
              {shown.map((p) => {
                ...
                return (
                  <label
                    ...
                    className={cn(
                      gridCols,
                      "min-h-[46px] w-full cursor-pointer border-b border-border/50 px-5 py-2 text-left transition-colors last:border-b-0 hover:bg-muted/40",
