...
export interface PluginInstallStatus {
  state?: string;
  error?: string | null;
  targets?: Array<{ name: string; state: string; log?: string[] }>;
}

// ── settings / env / guard / registry / self-update ─────
export interface AutostartStatus { on: boolean; unit?: string; gui?: boolean; }
export interface LanPanelStatus { enabled: boolean; host?: string; port?: number; }
...
export interface DshEnvStatus { binOk?: boolean; installed?: string; }
