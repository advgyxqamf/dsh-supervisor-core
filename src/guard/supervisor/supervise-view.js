  ...
  _orphanAudit() {
    ...
    try {
      ...
      const daemons = [
        ...
        { kind: 'lan-daemon', port: this._lanCtlPort(), active: () => this._lanDaemonActive(), managed: () => this._lanManaged(), want: () => this.lanDaemonEnabled() || !!(reg && reg.get('lan-daemon') && reg.get('lan-daemon').desired === 'running') },
      ];
