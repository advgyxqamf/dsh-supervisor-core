...
//   真实能力以 platform/os/index.js 的 capabilityProfile() 声明为准，并由
//   test/platform-capability-audit-test.js 强制与实现绑定。
...
class Supervisor {
  constructor(rawConfig, configPath) {
    ...
    this._shellWatchdogTimer = null;   // 桌面壳看护定时器
    ...
    const logCore = require('./platform/logcore').init({
      ...
      ctlPorts: { router: Number(this.config.routerCtlPort) || 43107, lan: Number(this.config.lanCtlPort) || 43108 },
      daemonLogs: {
    ...
    // 桌面壳更新安全网门面（状态/账本/健康/回退）。纯函数式模块：
    // **不持有 dist、不调用任何安装执行器** —— 与内核更新机制完全隔离（D6）。
    this.shellDomain = shellDomain;
   ...
   * 启动桌面壳看护（2026-09-11）。
   *
   * 为什么是守卫的职责：产品意图「壳关不掉」意味着壳只会在**崩溃**时消失；
   * 而壳无法自我监督（监督者随它一起死），守卫是抗重启的那个。
   * 覆盖三平台：修复前 Linux/macOS 完全没有壳自愈，Windows 的壳检查被条件屏蔽。
   *
   * 失败隔离：任何异常都不得影响守卫主循环 —— 看护是**增强**，不是依赖。
   */
  _startShellWatchdog() {
    if (this.config.shellWatchdog === false) {
      this.logger.info && this.logger.info('[shell-watchdog] 已按配置禁用');
      return;
    }
    try {
      this.shellWatchdog = createShellWatchdog({
        shell: this.shellDomain,
        pidlookup: pidlook, desktop: platform.desktop,
        logger: this.logger,
        events: this.events,
        config: this.config,
      });
      this._shellWatchdogTimer = setInterval(() => {
        Promise.resolve(this.shellWatchdog.tick()).catch(() => {});
      }, this.shellWatchdog.intervalMs);
      if (this._shellWatchdogTimer.unref) this._shellWatchdogTimer.unref();
      this.logger.info && this.logger.info('[shell-watchdog] 已启用（周期 ' +
        Math.round(this.shellWatchdog.intervalMs / 1000) + 's）');
    } catch (e) {
      this.logger.warn && this.logger.warn('[shell-watchdog] 初始化失败（不影响守卫）: ' + ((e && e.message) || e));
    }
  }
  ...
  shutdown() {
    ...
    if (this._shellWatchdogTimer) clearInterval(this._shellWatchdogTimer);
    if (this.api) {
      try {
        this.api.close();
      } catch {}
  ...
  // ── 本节已拆分 → guard/supervisor/settings-view.js（§7.6 结构性重构）──
  ...
  async _stopAllSandboxes() {
    ...
    for (const inst of sandboxes) {
      ...
      } catch (e) { this.logger.warn && this.logger.warn('shutdownAll stop sandbox ' + inst.id + ': ' + e.message); }
      try { if (inst.state) inst.state.phase = 'STOPPED'; } catch {}
    }
    if (sandboxes.length) { try { this.instances.save(); } catch {} }
  }
  ...
  /** 退出管家（契约 §4.1 冻结时序）：停全部被管对象 → 置 stopped → 回执。
   *  **绝不 systemctl stop/restart 自己**（守卫所属单元的所有者是外部——systemd + 壳，契约 §2）。
   *  守卫进程此后由壳（systemctl --user stop）或 systemd 停止。
   *  返回 { ok, sessionState } 供壳做退出握手。 */
  async shutdownAll() {
    ...
    this.logger.info('[session] 被管对象已全部停止；等待外部所有者（壳/systemd）停止守卫进程');
