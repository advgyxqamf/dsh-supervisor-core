...
class ManagedLifecycle {
  ...
  constructor(opts) {
    ...
    //   ⚠ 域 A 的真实计数**不在这里**：dsh 走 main-process.js 的 `restart_triggered` + `restartCount`
    ...
    // ⚠ 域 B 基础设施（router-daemon/lan-daemon）**不属此轴**——它们由保活路径无条件拉起，
  ...
  snapshot() {
    return {
      ...
      error: this.error,
      // （无 restartCount：域 A 计数不在本对象上，见构造期说明）
   ...
   *      于是 `POST /lifecycle/{id}/restart`（api/lifecycle.js 的 restart 分支）在「停不掉」或「起不来」时
