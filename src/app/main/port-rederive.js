...
// ═══════════════════════════════════════════════════════════════════════════
// app/main/port-rederive.js —— 原生 DSH 端口的**运行时再推导**（独立切面）。
//
// 从 app/main/process.js 拆出（R3 严值 DF-2：main/process.js ≤300；
//   DF-3：进程启停编排与「端口重派生 + 注册表更正」分离；DF-6：可独立 require）。
//
// 为什么单独成文件：本职责消费全局端口注册表（ports.register/release）并更正
//   config/dsh-main/relay 目标的**五处跟随**，与「spawn/停/杀」的进程生命周期正交；
//   由 app/assembly/facets.js 作为独立切面装到 host（成员名 _findManagedDshPort /
//   _applyMainPort 逐字不变——协作方接口表与 test/main-port-rederive-test.js 都依赖）。
//
...
function applyMainPort(host, newPort, pid) {
  ...
  //     （ports.js 的 register 固定写 'system:' + role），不是 'dsh-main'。写错会让释放变 no-op → 旧端口残留
