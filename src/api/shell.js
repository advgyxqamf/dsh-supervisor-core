...
// 域：桌面壳更新安全网 API（/shell/*）—— 2026-09-11
//
// 端点设计原则：
...
function handle(ctx) {
  ...
  // 产品语义：面板「检查更新」需对**内核与桌面壳一起检测**，本端点提供壳那一半。
  // 定位重申：内核不是壳的更新源；此处只**查版本**，安装仍由壳的门 0 完成。
  ...
  // 重启桌面壳（用于**应用壳更新**）：壳的自更新发生在壳启动时（门 0），
  // 因此「让壳用上新版本」= 让壳重新启动一次，门 0 会在新进程里完成检测/下载/验签/安装。
