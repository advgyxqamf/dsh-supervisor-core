...
// api —— 本地 HTTP API 门面（步骤 9 / DF-1：拆薄为「导出面 + 委托」）。
//
// 本文件只做两件事：re-export 网关 createServer（实现 → ./transport/server.js）与安全判定
// 三函数（实现 → ./security.js）。网关本体、静态托管、body 读取、域注册表、契约面分别归
// ./transport/server.js、./static.js、./transport/body.js、./router-table.js、./contract.js。
//
// 安全边界（三层，职责单一）：
...
// originAllowed / isLoopbackHost / isShellOrigin：供测试直接做行为断言——仅源码正则不够
//   （本仓已有「注释声称、代码没有」的先例 K6，正则也会被注释示例骗过）。
// 消费方：api-contract / lan-access-boundary / defects-batch-f（K6 直调 originAllowed）。
// ⚠ 步骤 9 后实现归 ./security.js，本文件只 re-export（保消费者零改动）。
