...
function createRelay(targetHost, targetPort, opts) {
   ...
   *  浏览器重连拉历史即可见完整结果。
   *
   *  ⚠ 2026-09-12 注释校准：此处原写「HOLD_MS 上限后清理（防无限挂起）」——
   *    但**全仓没有 HOLD_MS**（仅此一处提及），且下方实现明确写着「不做定时清理」。
   *    即旧注释声称了一条**并不存在**的上限，会让读者以为挂起已被限界。
   *
   *  **实际语义（有意取舍，2026-09 用户定稿）**：保持模式**不设时限**，
   *    只等上游「自然结束」（ur end/error）——理由是「无法靠超时值判断连接是否还有效，
   *    只有自然结束才可信」。
   *
   *  ⚠ 该取舍的**代价**（记录在案，便于日后判断是否要改）：
   *    客户端断开后，上游连接、res 对象与闭包在 DSH 自然结束前**不释放**；
   *    浏览器反复中断长会话时，relay 侧连接/内存会单调增长 ——
   *    若未来观察到内存问题，这里是第一个该看的地方。
   *
