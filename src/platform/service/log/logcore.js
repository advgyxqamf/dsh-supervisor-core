...
// ═══ DS-G4 生产装配注入（§4.2 反转法）═══
// platform 层代码字面量不得含业务域名词。本门面（进程日志装配的唯一入口）在装载时加载
// app/assembly/log-sources.js —— 该模块即「业务源名单」的唯一声明处（源名/短键/local/内部簿记类型）。
//  - 注入失败（文件缺失/抛错）不改变装配主流程：EventHub 退化为「仅本地源」，只是聚合面变窄。
//  - Node 模块缓存 ⇒ 幂等；app 模块不反向 require platform 业务标识，无循环依赖。
// ⚠ DS-G2 修正（2026-09-16）：**platform 不得出边到 app**。
//   原实现在此 try{require('app/assembly/log-sources')} 注入源名单 —— 方向反了（platform→app）。
//   改为**调用方注入**：compose.js（守卫进程）、router/daemon.js、relay/daemon.js（两个独立
//   daemon 进程）各自在 LogCore.init 之前 require('app/assembly/log-sources')。
//   未注入时 EventHub 退化为「仅本地源」（聚合面变窄，但进程内日志不受影响）。
 ...
 *  process 只要求「非空字符串」（DS-G4：platform 不枚举业务进程名——具体名字由 app/ 注入）。 */
function init(opts) {
